...
def synthesize(model, step, configs, vocoder, data_loader, control_values,log_filename,synthesized_wav_dir, reference_wav_dir, device):
    ...
    for idx, batch in enumerate(data_loader):
        batch = to_device(batch, device) #gpu
        with torch.no_grad():
            start = time.time() #gpu
    ...
    if mcd_scores or pesq_scores or inference_times:
        ...
        log_data(log_filename, f"Average MCD Score: {average_mcd:}, Average PESQ Score: {average_pesq:}, Average RTF: {average_rtf:}")
...
if __name__ == "__main__":
    ...
    # Get model
    model = get_model(args, configs, device, train=False) #error
